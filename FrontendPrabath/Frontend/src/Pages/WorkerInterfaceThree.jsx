...
function WorkerInterfaceThree() {
  ...
  const handleAddOrder = async () => {
    try{
      const response = await axios.put('http://localhost:8090/OnGoingOrder/WorkstationThreeFetch/'+ orderId1 +'/'+ numberOfItemsAdd)
    }
    catch(error){
  ...
  const handleSubmitPass =async (e) => {
    try{
      const response = await axios.put('http://localhost:8090/OnGoingOrder/WorkstationThreePass/'+ orderId1 +'/'+ numberOfItemsPass)
    }
    catch(error){
        alert("Enter valid Amount");
    }
